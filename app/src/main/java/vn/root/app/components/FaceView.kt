...
class FaceView(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
		...
		context?.theme?.obtainStyledAttributes(attrs, R.styleable.FaceView, 0, 0)?.apply {
			try {
				mFaceColor = getColor(
					...
					ContextCompat.getColor(context, R.color.md_theme_primaryFixed)
				)
				mEyeColor = getColor(
	...
	private fun onDrawBgFace(canvas: Canvas) {
		...
		// Draw stroke
		paint.color = ContextCompat.getColor(context, R.color.md_theme_primary)
		...
		paint.color = mFaceColor ?: ContextCompat.getColor(context, R.color.md_theme_primaryFixed)
	...
	private fun onDrawEyes(canvas: Canvas) {
		paint.color = mEyeColor ?: ContextCompat.getColor(context, R.color.md_theme_primary)
		paint.style = Paint.Style.FILL
		val radius = mSize / 8f
	...
	private fun onDrawMouth(canvas: Canvas) {
		paint.color = mMouthColor ?: ContextCompat.getColor(context, R.color.md_theme_primary)
		paint.style = Paint.Style.FILL
		val radius = mSize / 8f
		val mouthPath = Path()
